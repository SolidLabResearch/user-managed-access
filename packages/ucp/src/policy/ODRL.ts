 ...
 * @returns 
 */
export function createConstraintQuads(constraint: UCPConstraint, ruleIRI?: string): { quads: Quad[], constraintIRI: string } {
    const quads: Quad[] = []
    ...
    switch (constraint.type) {
        case "temporal": // maybe have as type something more semantically defined?
            ...
            quads.push(quad(namedNode(constraintIRI), ODRL.terms.rightOperand, literal((constraint.value as Date).toISOString(), XSD.terms.dateTime)));
            if (ruleIRI) {
