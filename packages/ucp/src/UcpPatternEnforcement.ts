import { Reasoner, rdfTransformStore } from "koreografeye";
...
export class UcpPatternEnforcement implements UconEnforcementDecision {
  ...
  constructor(
    private uconRulesStorage: UCRulesStorage, 
    private koreografeyeOdrlRules: string[], 
    private reasoner: Reasoner, 
    private executor: IPolicyExecutor
  ) {}
   ...
   * The conclusion of the reasoning results into a graph with 0..n policies, which are then executed by the plugins.
   * This executioner and plugins are given by the {@link IPolicyExecutor}.
   * If there are zero policies in the conclusion, a request will be sent to the Resource Owner. 
   * Furthermore, the user is notified that more information is needed. TODO:
...
export interface UconEnforcementDecision {
  /**
   * Calculates the modes granted (i.e. the `actions`) based on the request and the configured Usage Control Rules
   * and how they are interpreted.
   * 
   * @param request A parsed Usage Request containing `who` wants to perform `which action` on a given `resource`
  ...
  /**
   * Calculates the modes granted (i.e. which `actions`) based on the request and the configured Usage Control Rules
   * and how they are interpreted.
