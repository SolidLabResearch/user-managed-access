 ...
 * and then registers a PAT for every account that has a WebID.
 *
 * The intended goal is for this is to ensure seeded accounts automatically get a PAT.
  ...
  protected async initialize(): Promise<void> {
    ...
    for await (const { webId, accountId } of this.accountStorage.entries(WEBID_STORAGE_TYPE)) {
      ...
      if (await this.accountStore.getSetting(accountId, ACCOUNT_SETTINGS_AS_TOKEN)) {
        this.logger.debug(`Account ${accountId} with WebID ${webId} already has a PAT`);
        continue;
      }
      ...
      const pat = await this.umaClient.generatePat(webId, issuer);
      this.logger.info(`Generated PAT ${pat} for WebID ${webId}`);
