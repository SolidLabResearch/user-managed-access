...
export class OwnerUtil {
   ...
   */
  public async findUmaSettings(webId: string): Promise<{ issuer?: string, pat?: string }> {
    ...
    const pat = await this.accountStore.getSetting(accountId, ACCOUNT_SETTINGS_AS_TOKEN);
    return { pat, issuer };
  }
}
