...
import { fetchThroughUMA, initContainer } from "./util";
import { VerifiableCredential } from "./Types";
import { Session } from "@inrupt/solid-client-authn-browser";

// export async function readCredentialsDirectory(session: Session) {
...
//       console.error('User not logged in correctly.')
//       return [];
//     }

//     const { data } = await fetchThroughUMA(container, session.info.webId, session.fetch)
  
//     const parsed = new Parser({baseIRI: container}).parse(data)
...
export async function readCredentialsDirectory(session: Session) {
  const res = await fetch('./credentials/age-credential.jsonld');
  const credential = await res.json()
  console.log('Credential', credential)
  credential.location = "http://localhost:3000/ruben/credentials/age-credential"
