...
import {ExportKeyParameters, IDidDocument, K, SerializedKeyPair, SignParameters, VerifyParameters} from "../interfaces";
//@ts-ignore
 ...
 * @param password
 */
export async function createKey(webId: string, password: string, keyUrl: string): Promise<K> {
...
export async function addFileToContainer(
    ...
    if (publicAccess!!) {
        await universalAccess.setPublicAccess(serverResourceInformation.internal_resourceInfo.sourceIri, publicAccess!, {fetch: authFetch})
    }
